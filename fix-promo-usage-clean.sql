...
-- This version can be run in any SQL interface (no psql meta-commands)

-- 1. Check current promotions and their usage
SELECT 
    'BEFORE FIX - Current promotions:' as status,
    code, 
    description, 
    discount_percent,
    usage_limit,
...
-- 2. Check promotion usage by user
SELECT 
    'BEFORE FIX - Usage by user:' as status,
    p.code,
    p.description,
    pu.user_id,
    pr.email,
...
-- 3. Drop the existing problematic function if it exists
DROP FUNCTION IF EXISTS record_promotion_usage(UUID, UUID, UUID);

-- 4. Create a proper record_promotion_usage function
...
-- 5. Reset times_used to match actual unique user usage
UPDATE promotions 
SET times_used = (
...
-- 6. Create test promotions with different per-user usage limits
...
-- 7. Check the results
SELECT 
    'AFTER FIX - Updated promotions:' as status,
    code, 
    description, 
...
-- 8. Show explanation of the new system
SELECT 
...
SELECT 'SUCCESS: Promotion system updated to per-user usage limits!' as result; 